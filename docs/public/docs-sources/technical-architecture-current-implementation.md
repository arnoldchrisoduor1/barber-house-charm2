...
> **Production target:** Use the split doc set **[README.md](./README.md)** (overview **00**, data **01**, backend **02**, frontend **03**, nav matrix **04**) and **[full-stack-implementation-master-plan.md](./full-stack-implementation-master-plan.md)** for CI/CD and phases. The blueprint **Tech Stack** in [haus-of-grooming-system-blueprint.md](./haus-of-grooming-system-blueprint.md) matches that target. **This file** remains a factual snapshot of the **prototype** (Vite + hosted PostgreSQL client) for historical comparison only.

**Scope:** This document is a **historical snapshot** of the **Phase 0** prototype: **code and data paths** in that UI as a **UI and data-model reference implementation**, not the final production stack. The **implemented stack** here is a **single-page application** talking **directly to PostgreSQL** via a hosted client (auth, RLS, edge functions in that environment) — **not** the Laravel + Next.js + Redis + Soketi architecture used for the shipped product.

---

## 1. Executive summary

The repository ships a **multi-tenant grooming / beauty operations dashboard** and **customer portal** as a **Vite + React 18 + TypeScript** client. The Phase 0 hosting layer provided authentication, PostgreSQL persistence, storage buckets, optional realtime, and **serverless functions** for scheduled-style workloads (for example SMS reminders via Twilio through Lovable’s connector gateway).
...
flowchart TB
  ...
  subgraph supa["Managed backend (Phase 0)"]
    Auth["GoTrue / Auth\nJWT in localStorage"]
    API["PostgREST\nREST over Postgres"]
...
| Auth | JS auth client (Phase 0) | `signUp` / `signInWithPassword`, session persistence |
| Data access | `db.from(table)` wrapper in `src/lib/db.ts` | Thin alias over the generated client (typed schema in `types.ts`) |
| Optional OAuth | `@lovable.dev/cloud-auth-js` | Lovable cloud auth bridge |
| DB | PostgreSQL (hosted) | Tenancy, bookings, CRM, inventory, etc. |
| Async / integrations | Serverless functions (Phase 0 host) | Reminder job calling Twilio |
...
3. `BrowserRouter`  
4. `AuthProvider` — subscribes to the client `auth.onAuthStateChange` stream  
...
`useAuth` listens to client auth state and exposes `user`, `session`, `loading`.
...
```mermaid
flowchart TD
  U[User submits signUp] --> SA[Auth service: create user]
...
`PublicBookingPage` creates `customers` (by phone dedupe) and inserts `bookings` / `booking_services` **without** passing `organization_id` in the shown insert payload (unlike staff `BookingPage`). Whether this is safe depends on **live RLS policies and column defaults** on the deployed database (not fully represented in the four SQL files under `migrations/`). Treat this as a **mandatory review item** when hardening multi-tenant isolation.
...
- **Canonical column-level schema** for the TypeScript app: `types.ts` / generated client types.  
- **Partial DDL** checked into git: `migrations/*.sql` (organizations, subscriptions, branding, audit, reviews, schedules, waitlist, notifications, storage policies, etc.).  

Many tables appear in `types.ts` but **do not** have `CREATE TABLE` in the repo migrations (they may exist only on the hosted database for Phase 0). Treat migrations in git as **incomplete** relative to production until a full dump or migration history is exported.
...
sequenceDiagram
  ...
  participant SB as PostgREST / SQL API
...
`useDemoFallback` returns **synthetic** arrays from `lib/demoData.ts` when the data client returns empty arrays — **without** an on-screen banner.
...
This section maps **this repository’s actual dependencies** to operational cost and failure modes. Figures are **order-of-magnitude planning anchors** in KES; tune to your database + auth host, Twilio volume, and hosting choice.

### 10.1 Billable components

| Component | What drives cost | Cold start / choke notes |
|-----------|------------------|---------------------------|
| **Hosted DB + API** (Phase 0) | Database size, MAU auth, stored data egress, edge invocations, Storage | PostgREST is always warm; **serverless functions** cold start (hundreds of ms to few s) on scale-to-zero tiers; **Realtime** connections per tab |
...
| **Auth email** | Provider default or custom SMTP | Signup confirmation deliverability affects funnel |
...
3. **`db.from` escape hatch** — `(client as any).from(table)` disables compile-time table checks; schema renames break at runtime.  
...
|-------|--------------|------------------------|
| Now | SPA + hosted DB/API | Already multi-tenant at DB |
| Growth | Add read replicas; cache subscription in edge middleware | Introduce dedicated queue worker for SMS batches |
...
| Realtime | Hosted realtime for `notifications` (Phase 0) | Soketi / Pusher for chat-scale fanout |

### 10.4 Cost scenario table (illustrative — engineering planning)

| Stage | Monthly band (KES) | Users / orgs (indicative) | Notes |
|-------|-------------------|---------------------------|--------|
| Dev / pilot | ~5k–15k | Tens | Low-tier hosted DB + minimal SMS |
| Single-region MVP | ~20k–45k | Hundreds | Paid DB tier, SMS volume, small always-on functions |
...
Strategic gap vs the PDF’s AWS Lightsail table: **Bundled** DB + Auth + API on a PaaS; you trade **ops simplicity** for **less granular** cost line items until you outgrow the platform or negotiate enterprise.
...
1. Regenerate the TypeScript / client types after every schema migration.  
2. Export full SQL migration history into `migrations/` so git matches production.  
...
| Field | Value |
|-------|--------|
| Repository (historical) | Phase 0 prototype (Vite + hosted PostgreSQL client) — archived |
| Primary entrypoints | `src/App.tsx`, `src/lib/db.ts`, `src/hooks/useAuth.tsx`, `src/hooks/useSubscription.ts` |
| Database artifacts | `migrations/`, shared `types.ts` / generated client |
| Serverless / jobs | e.g. `functions/send-reminders` (Phase 0 host) |
